...
from app.dependency import getUsers

router = APIRouter()

@router.post('/create')
def create_teacher(teacherRespone: TeacherRespone, db: Session = Depends(get_db), userPermission = Depends(getUsers)):
...
def getById(user_id, db: Session = Depends(get_db),userPermission = Depends(getUsers)):
    return getByIdTeacher(user_id,db,userPermission)
